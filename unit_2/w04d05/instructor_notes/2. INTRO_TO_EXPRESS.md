...
**TLDR;** - a server is just software that runs on a computer that handles requests and responses.

![servers](https://techcrunch.com/wp-content/uploads/2012/01/datacenter.jpg?w=990&crop=1)

## Explain what Node.js is and why it's useful

- Node is just a command line application that reads a JavaScript file and executes it within the context of the terminal - until recently, JavaScript could only be executed within a browsers
- Node allows JavaScript to become the first programming language that can be executed in both the browser and in a terminal application - Makes it easier to find a developer who can build all aspects of a web application
- Asynchronous - Uses event handlers (just like click, hover, etc) so you can tell an application to run code while waiting for other commands to finish executing - previously, long running commands like updating a database would have to finish before the application could continue running

## Install Node packages

- We'll use node.js to build simple servers that will respond to browser requests.
...
It's totally ok to edit this file - for example, if I forgot to put myself as the author I could add it as a string. If I didn't like my project name, I could update it too. **GOTCHA** be careful to keep this a proper JSON and keep track of your strings/double quotes/commas or else you will get errors and your code will not run.
...
- We can also open up our dev tools and go to the `Network` tab to see our request:

![](https://i.imgur.com/djjysad.png)

Here we see

- where the request was made `http://localhost:3000`
- the type of reqeust `GET`
- the status code `304` - ok, not modified

We can also learn about the response we got from the server

![](https://i.imgur.com/TmxgP7z.png)

- The content length is the string `hello`
- we see other details that it is either text or html, the charset is utf-8, the date, and that it is powered by express

**Rabbit hole**

- we can scroll further down or open portions of this information and find a lot more! But we're just going to focus on what we need to build the basics today
...
It's a good idea to shut down your server when you stop working (let the computer go to sleep/start a new lab/hw). Otherwise it can be annoying to shut down the server.

### Can't shut down your server

Sometimes you lose your terminal window that was running node and can't use control c to shut it down.

You'll need to use terminal to find the process number of the program you're trying to terminate with

-`ps -A |grep nodemon`

- This will show the process number
- Then use the kill command to end the process
- `kill 4517`

![Thank you Alex Roland](./grep_n_kill_nodemon.png)

If that doesn't work, you'll need to reboot your computer.

## Use nodemon to restart your sever when your code changes

_You will likely need your computer password to install this_
