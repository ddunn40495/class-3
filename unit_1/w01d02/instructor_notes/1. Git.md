...
1. Go to a suitable location in the terminal (let's do `~/dev/`) and type `git clone ` and paste the https url (e.g.`git clone https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Moss.git`)
...
1. Add original remote repo with `git remote add upstream https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Moss.git`
